...
check(
    "short history not extrapolated",
    r["status"] in ("insufficient_data", "collecting"),
    r["status"],
)
...
check(
    "baseline near 243", 235 < r["baseline_capacity"] < 250, str(r["baseline_capacity"])
)
...
check(
    "years_remaining plausible",
    1.5 < r["years_remaining"] < 5,
...
check(
    "at_end_of_life or ok-with-low-remaining",
    r["status"] in ("at_end_of_life", "ok"),
    r["status"],
)
...
check(
    "counter reset does not invent a negative life",
...
check(
    "window capacity near truth",
    all(235 < v < 250 for _, v in w),
    str([round(v) for _, v in w][:4]),
)
...
check(
    "lifetime avg handles zero regens",
...
check(
    "correct unit: cycle age == calendar age",
    abs(resin.cycle_age_years(CORRECT, 4.902) - 747 / 365.25) < 0.1,
    str(round(resin.cycle_age_years(CORRECT, 4.902), 2)),
)
check(
    "correct unit: acceleration ~1.0",
    abs(resin.cycle_age_acceleration(CORRECT, 4.902) - 1.0) < 0.05,
    str(round(resin.cycle_age_acceleration(CORRECT, 4.902), 3)),
)
check(
    "correct unit: no excess cycles",
    resin.excess_cycles_lifetime(CORRECT, 4.902) < 1.0,
    str(round(resin.excess_cycles_lifetime(CORRECT, 4.902), 2)),
)

check(
    "under-regenerating clamps excess at zero",
    resin.excess_cycles_lifetime(
        {"total_regens_since_install": 50, "days_since_install": 747}, 4.902
    )
    == 0.0,
)
check("missing expected interval is None", resin.cycle_age_years(REAL, None) is None)
check("zero expected interval is None", resin.cycle_age_years(REAL, 0) is None)
check("missing datapoints is None", resin.cycle_age_acceleration({}, 4.902) is None)
